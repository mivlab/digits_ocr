...
class Crawler:
    ...
    # 保存图片
    def save_image(self, img_path,rsp_data, word):
        # if not os.path.exists("./" + word):
        #     os.mkdir("./" + word)
        if not os.path.exists(os.path.join(img_path,word)):
            os.makedirs(os.path.join(img_path,word))
        # 判断名字是否重复，获取图片长度
        self.__counter = len(os.listdir(os.path.join(img_path,word))) + 1
        for image_info in rsp_data['data']:
            try:
                ...
                # 保存图片
                filepath = '%s/%s/%s' % (img_path,word, str(self.__counter) + str(suffix))
                ...
                print("图片保存路径："+ img_path + "/" + word)
                print("图片+1,已有" + str(self.__counter) + "张图片")
                self.__counter += 1
        return

    # 开始获取
    def get_images(self, img_path, word):
        ...
        while pn < self.__amount:
            ...
            try:
                ...
                self.save_image(img_path,rsp_data, word)
                # 读取下一页
                print("下载下一页")
                pn += self.__per_page
        print("下载任务结束")
    ...
    def start(self, img_path, word, total_page=1, start_page=1, per_page=30):
        """
        爬虫入口
        ...
        self.get_images(img_path, word)


if __name__ == '__main__':
    if len(sys.argv) > 1:
        parser = argparse.ArgumentParser()
        parser.add_argument("-ip", "--img_path", type=str, help="图片保存路径", required=True)
        ...
        crawler.start(args.img_path, args.word, args.total_page, args.start_page, args.per_page)  # 抓取关键词为 “美女”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
    else:
        # 如果不指定参数，那么程序会按照下面进行执行
        crawler = Crawler(0.05)  # 抓取延迟为 0.05

        # img_path:图片保存路径
        img_path = r'D:\practice\detection\Digit_recognition\data\bg'
        crawler.start(img_path, '风景', 1, 2, 30)  # 抓取关键词为 “风景”，总数为 10 页，开始页码为 2，每页30张（即总共 10*30=300 张）
